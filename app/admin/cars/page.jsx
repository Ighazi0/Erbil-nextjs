...
import Switch from "@/app/admin/cars/switch.jsx";
...
const CarsPage = () => {
    ...
    try {
      ...
      const carsData = await Promise.all(
        carsSnapshot.docs.map(async (doc) => {
          ...
          const [typeDoc, brandDoc, modelDoc] = await Promise.all([
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      showSnackbar(
        intl.formatMessage({ id: "car_deleted_success" }),
        "success"
      );
      fetchCars();
    } catch (error) {
  ...
  const filteredCars = useMemo(() => {
    return cars.filter((car) => {
      ...
      const matchesType = !typeFilterLower || carType === typeFilterLower;

      const matchesStatus =
        !statusFilterLower || carStatus === statusFilterLower;
        ...
        <div className="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
          <div>
            ...
            >
              <FormattedMessage
                id="cars_management"
                defaultMessage="Cars Management"
              />
            </Typography>
          </div>
          ...
          >
            <TextField
              ...
              placeholder={intl.formatMessage({
                id: "search_cars_placeholder",
                defaultMessage: "Search by name or model...",
              })}
              value={searchQuery}
