...
import { Card, Table, Button, Badge, Modal, Form, InputGroup, Row, Col } from 'react-bootstrap';
import { FaEye, FaSearch, FaCar, FaUser, FaCalendarAlt, FaMoneyBillWave, FaClock } from 'react-icons/fa';
        ...
        >
            ...
            <Modal.Header closeButton className="bg-opacity-10 border-bottom-0 px-4">
                <Modal.Title className="d-flex align-items-center">
                    <span className="text-primary p-2 rounded-circle me-2 d-flex align-items-center justify-content-center" style={{ width: '40px', height: '40px' }}>
                        <FaCar size={20} />
                    </span>
                    <div>
                        <div className="d-flex align-items-center gap-2">
                            <h5 className="mb-0">Order #{order.id.slice(0, 8)}</h5>
                            {getStatusBadge(order.payment, order.from, order.to)}
                        </div>
                        <small className="text-muted">Created on {formatDate(order.createdAt)}</small>
                    </div>
                </Modal.Title>
            </Modal.Header>
            <Modal.Body className="px-4 py-0" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
                            ...
                            <Card.Body>
                                ...
                                <div className="ps-4">
                                    <div className="mb-3">
                                        ...
                                        <div className="fw-semibold">{order.carDetails?.brandName} {order.carDetails?.modelName}</div>
                                    </div>
                                    <div className="mb-3">
                                        <div className="text-muted mb-1">Name</div>
                                        <div className="fw-semibold">{order.carDetails?.name || 'N/A'}</div>
                                    </div>
                                    <div className="mb-3">
                                        <div className="text-muted mb-1">Type</div>
                                        <div className="fw-semibold">{order.carDetails?.type || 'N/A'}</div>
                                    </div>
                                    <div>
                                        <div className="text-muted mb-1">Location</div>
                                        <div className="fw-semibold">{order.carDetails?.location || 'N/A'}</div>
                                    </div>
                                </div>
                            </Card.Body>
                        </Card>
                        ...
                        <Card className="h-100 border-0 shadow-sm">
                            <Card.Body>
                                ...
                                <div className="ps-4">
                                    <div className="mb-3">
                                        <div className="text-muted mb-1">Name</div>
                                        <div className="fw-semibold">{order.userDetails?.name || 'N/A'}</div>
                                    </div>
                                    <div className="mb-3">
                                        <div className="text-muted mb-1">Email</div>
                                        <div className="fw-semibold">{order.userDetails?.email || 'N/A'}</div>
                                    </div>
                                    <div className="mb-3">
                                        <div className="text-muted mb-1">Phone</div>
                                        <div className="fw-semibold">{order.userDetails?.phone || 'N/A'}</div>
                                    </div>
                                    <div>
                                        <div className="text-muted mb-1">Address</div>
                                        <div className="fw-semibold">{order.userDetails?.address || 'N/A'}</div>
                                    </div>
                                </div>
                            </Card.Body>
                        </Card>
                                    ...
                                    <Col md={4}>
                                        <div className="p-3 bg-light rounded">
                                            ...
                                            <div className="d-flex align-items-center">
                                                ...
                                                <span className="fw-semibold">{formatDate(order.from)}</span>
                                            </div>
                                        </div>
                                    </Col>
                                    <Col md={4}>
                                        <div className="p-3 bg-light rounded">
                                            ...
                                            <div className="d-flex align-items-center">
                                                ...
                                                <span className="fw-semibold">{formatDate(order.to)}</span>
                                            </div>
                                        </div>
                                    </Col>
                                </Row>
                                <div className="mt-4">
                                    <div className="p-3 bg-light rounded mb-3">
                                        ...
                                        <div className="d-flex align-items-center">
                                            ...
                                            <span className="fw-semibold">{order.carDetails?.price || 0} AED / day</span>
                                        </div>
                                    </div>
                                    <div className="p-3 bg-opacity-10 rounded d-flex align-items-center justify-content-between">
                                        <div className="d-flex align-items-center">
                                            ...
                                            <span className="text-primary fw-semibold">Total Amount</span>
                                        </div>
                                        ...
                                        <Col md={6}>
                                            <div className="mb-3">
                                                ...
                                                <div className="fw-semibold">
                                                    <Badge bg={order.payment === 'success' ? 'success' : 'warning'}>
            ...
            const ordersData = await Promise.all(ordersSnapshot.docs.map(async (doc) => {
                ...
                if (data.car) {
                    try {
                        ...
                        let brandName = '';
                        if (carData.brand) {
                            const brandRef = typeof carData.brand === 'string' ? doc(db, carData.brand) : carData.brand;
                        ...
                        let modelName = '';
                        if (carData.model) {
                            const modelRef = typeof carData.model === 'string' ? doc(db, carData.model) : carData.model;
                        ...
                        let typeName = '';
                        if (carData.type) {
                            const typeRef = typeof carData.type === 'string' ? doc(db, carData.type) : carData.type;
                        ...
                        let locationName = '';
                        if (carData.location) {
                            const locationRef = typeof carData.location === 'string' ? doc(db, carData.location) : carData.location;
                        ...
                        carDetails = {
                            ...
                            price: typeof carData.price === 'number' ? Number(carData.price) : 0,
                ...
                if (data.user) {
                    try {
                        const userRef = typeof data.user === 'string' ? doc(db, data.user) : data.user;
                ...
                return {
                    id: doc.id,
                    days: data.days || 0,
                    totalPrice: typeof data.totalPrice === 'number' ? Number(data.totalPrice) : 0,
                ...
                <Card.Header className="bg-white py-3 border-bottom">
                    <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        <div>
                            ...
                            <small className="text-muted">Manage and view all rental orders</small>
                        </div>
                    ...
                    <Table responsive hover className="mb-0">
                        <thead>
                            <tr className="bg-light">
                                ...
                                <th className="fw-semibold text-primary text-center py-3">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            {filteredOrders.length === 0 ? (
                                <tr>
                                    <td colSpan="8" className="text-center py-5">
                                        <div className="text-muted">
                                            ...
                                            <div>{searchQuery ? "No orders found matching your search" : "No orders found"}</div>
                                        </div>
                                    </td>
                                </tr>
                            ) : (
                                filteredOrders.map(order => (
                                    <tr key={order.id} className="align-middle">
                                        <td className="px-4">
                                            <div className="d-flex align-items-center">
                                                <div className="p-2 rounded me-2">
                                            ...
                                            {order.carDetails ? (
                                                <div>
                                                    <div className="fw-semibold text-dark">
                                                        {order.carDetails.brandName} {order.carDetails.modelName}
                                                    </div>
                                                    <small className="text-muted">{order.carDetails.name}</small>
                                                </div>
                                            ) : (
                                                'N/A'
                                            )}
                                        ...
                                        <td>
                                            {order.userDetails ? (
                                                <div>
                                                    <div className="fw-semibold text-dark">{order.userDetails.name}</div>
                                        ...
                                        <td>{getStatusBadge(order.payment, order.from, order.to)}</td>
