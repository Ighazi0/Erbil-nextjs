...
export default function Header1({ scroll, isMobileMenu, handleMobileMenu, handleToggle1, isToggled1, handleToggle2, isToggled2, handleToggle3, isToggled3 }) {
    ...
    const [codes, setCodes] = useState([{ code: 'AED', rate: 1 }, { code: 'USD', rate: 0.27 }, { code: 'IQD', rate: 355.91 }]);
                            ...
                            <div className="col-lg-12">
                                <div className="header-style2 inner-container flex justify-space align-center">
                                    <div className="logo-box flex">
                                        <div className="logo"><Link href="/"><img src="/assets/images/logo/eblpng.png" alt="Logo" /></Link></div>
                                    </div>
                                    ...
                                    <div className="flex align-center">
                                        <div className="d-flex px-lg-0">
                                            <div className="lang-menu position-relative text-center fw-bold" style={{ width: '40px' }} ref={closeCode}>
                                                <div className="selected-lang d-flex align-items-center justify-content-end cursor-pointer" role="button" onClick={toggleCodeMenu}>
                                                    <span>{code}</span>
                                                </div>
                                                <ul className={`dropdown-menu position-absolute end-0 mt-2 bg-white border rounded shadow ${openCode ? 'show' : ''}`} style={{ width: '120px' }}>
                                                    {codes.map(e => (<li className="dropdown-item d-flex align-items-center cursor-pointer" role="button" onClick={() => handleCodeChange(e.code, e.rate)}>
                                                        <span>{e.code}</span>
                                                    </li>))}
                                                </ul>
                                            </div>
                                        </div>
                                        <div className="d-flex px-lg-0 px-md-5" style={{ paddingRight: '3rem'}}>
                                            <div className="lang-menu position-relative text-center fw-bold" style={{ width: '40px' }} ref={closeSearch}>
                                                ...
                                                <ul className={`dropdown-menu position-absolute end-0 mt-2 bg-white border rounded shadow ${openLang ? 'show' : ''}`} style={{ width: '120px' }}>
                                                    ...
                                                    <li className="dropdown-item d-flex align-items-center cursor-pointer" role="button" onClick={() => handleLanguageChange('kr')}>
                                                        <span>Kurd</span>
                                                    </li>
                                                </ul>
                                            </div>
                                        ...
                                        {user ? (
                                            <div className="header-account register ml--18" ref={userMenuRef}>
                                                <div className="flex align-center position-relative">
                                                    ...
                                                    <ul className={`dropdown-menu position-absolute end-0 mt-2 bg-white border rounded shadow ${openUserMenu ? 'show' : ''}`} style={{ width: '200px', top: '100%' }}>
                                                        <li>
                                    ...
                                    <div className={`mobile-nav-toggler mobile-button ${lang === 'ar' ? 'position-left' : 'position-right'}`} onClick={handleMobileMenu}>
                                        <span />
                                    </div>
                                </div>
                            </div>
                    ...
                    <nav className="menu-box">
                        ...
                        <div className="bottom-canvas">
                            ...
                            {!user ? (
                                ...
                                <div className="help-bar-mobie login-box pt-2">
                                    <button onClick={handleLogout} className="fw-7 category border-0 bg-transparent">
